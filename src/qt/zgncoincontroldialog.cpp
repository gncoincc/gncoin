...
#include "ui_zgncoincontroldialog.h"
...
std::list<std::string> ZGncoinControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZGncoinControlDialog::listMints;

ZGncoinControlDialog::ZGncoinControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZGncoinControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZGncoinControlDialog::~ZGncoinControlDialog()
{
    delete ui;
}

void ZGncoinControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZGncoinControlDialog::updateList()
{
...
void ZGncoinControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZGncoinControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZGncoin_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZGncoinControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZGncoinControlDialog::GetSelectedMints()
{
...
void ZGncoinControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
