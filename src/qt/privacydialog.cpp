...
#include "zgncoincontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zGNC ought to be enough for anybody." - Bill Gates, 2017
    ui->zGNCpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzGNCSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzGNC->setEnabled(false);
        ui->pushButtonMintzGNC->setToolTip(tr("zGNC is currently disabled due to maintenance."));

        ui->pushButtonSpendzGNC->setEnabled(false);
        ui->pushButtonSpendzGNC->setToolTip(tr("zGNC is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zGNCpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzGNC_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zGNC is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zGNC...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zGNC in ") + 
...
void PrivacyDialog::on_pushButtonSpendzGNC_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zGNC is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zGNC
        sendzGNC();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zGNC
    sendzGNC();
}

void PrivacyDialog::on_pushButtonZGncoinControl_clicked()
{
    ZGncoinControlDialog* zGncoinControl = new ZGncoinControlDialog(this);
    zGncoinControl->setModel(walletModel);
    zGncoinControl->exec();
}

void PrivacyDialog::setZGncoinControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzGncoinSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzGNC()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Gncoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zGNCpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zGNC is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " GNC </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zGNC</b>";
    ...
    // use mints from zGncoin selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZGncoinControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZGncoinControlDialog::GetSelectedMints();
    }
    ...
    // Clear zgncoin selector in case it was used
    ZGncoinControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zGncoin Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Gncoin, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zGncoin Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zGNC </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zGNC "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zGNC "));
    ui->labelzGNCAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzGNCSyncStatus->setVisible(fShow);
}
